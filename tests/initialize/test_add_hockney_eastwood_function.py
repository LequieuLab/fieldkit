...
def test_2d():
    ...
    assert(np.sum(field.data) ==  approx(2.0, abs=1e-4)) # confirm that integral = 2 (since 2nd added function to field)
    ...
    assert(np.sum(field.data) ==  approx(3.0, abs=1e-4)) # confirm that integral = 3 (since 3rd added function to field)
